...
namespace lrender
{
        ...
        ,length_(0)
    {
        ...
        if(ExpandSize<capacity_){
            ...
            buffer_.elements_ = GFX_NEW Char[capacity_];
    ...
    String::String(const Char* str)
    {
        ...
        ::memcpy(buffer, str, sizeof(Char)*length);
    }
    ...
    String::String(const String& rhs)
    {
        ...
        initBuffer(length);
        Char* buffer = getBuffer();
        ::memcpy(buffer, rhs.c_str(), sizeof(Char)*length);
    }
    ...
    {
        if(ExpandSize<capacity_){
            GFX_DELETE_ARRAY(buffer_.elements_);
        }
        capacity_ = 0;
    }
...
#ifdef _MSC_VER 
        s32 length = ::_vscprintf(format, args);
#else
    ...
    void String::expandBuffer(s32 length)
    {
        ...
        Char* elements = GFX_NEW Char[capacity];
        Char* buffer = getBuffer();
    ...
    s32 compare(const String& str0, const String& str1)
    {
        ...
        return ::strncmp(str0.c_str(), str1.c_str(), length);
    }
    ...
    {
        LASSERT(0<=capacity_);
        elements_ = GFX_NEW Char[capacity_];
        elements_[0] = CharNull;
    }

    StringBuilder::~StringBuilder()
    {
        GFX_DELETE_ARRAY(elements_);
    }
    ...
    void StringBuilder::swap(StringBuilder& rhs)
    {
        lrender::swap(capacity_, rhs.capacity_);
        lrender::swap(length_, rhs.length_);
        lrender::swap(elements_, rhs.elements_);
    }

    void StringBuilder::expand(s32 capacity)
    {
        Char* elements = GFX_NEW Char[capacity];
        ::memcpy(elements, elements_, sizeof(Char)*capacity_);
        GFX_DELETE_ARRAY(elements_);
        capacity_ = capacity;
        elements_ = elements;
    }
